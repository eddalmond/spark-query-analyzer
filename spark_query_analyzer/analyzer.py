...
from spark_query_analyzer.display_utils import error_card, format_diagnostics
from spark_query_analyzer.narrative_explainer import NarrativeExplainer, NarrativeResult
...
class Finding:
    ...
    config_snippet: str | None = None  # copy-ready Spark conf set() block
...
def run_analysis(
    ...
    from spark_query_analyzer.streaming_analyser import format_streaming_diagnostics, run_streaming_analysis
    ...
    from spark_query_analyzer.aqe_checker import build_recommendations, read_aqe_config
    ...
    if not dry_run:
        ...
        aqe_cfg = read_aqe_config(spark)
        skew_findings = run_post_execution_skew_analysis(spark, sql=sql, aqe_enabled=aqe_cfg.adaptive_enabled)
        result.skew_findings = skew_findings
        for sf in skew_findings:
            detail = None
            if sf.metrics:
                detail = (
                    f'max={sf.metrics.max_task_duration_ms / 1000:.1f}s | '
...
def parse_plan(plan_text: str, query: str = '') -> AnalysisResult:
    ...
    # Pre-scan for broadcast join indicators so we suppress false MISSING_BROADCAST
    has_broadcast = 'BroadcastExchange' in plan_text or 'BroadcastHashJoin' in plan_text
    ...
    for i, line in enumerate(lines):
        ...
        if 'Exchange' in stripped and not has_broadcast and i + 1 < len(lines):
            next_lines = '\n'.join(lines[i + 1 : i + 5])
        ...
        if stripped.startswith('+- ') or stripped.startswith('* '):
            ...
            has_filter_in_bracket = any('Filter' in b for b in re.findall(r'\[[^\]]+\]', stripped))
            if scan_match and not has_filter_in_bracket:
                ...
                if table != '<unknown>' and table not in ['', ' ']:
                    result.findings.append(
                        Finding(
                            ...
                            suggestion='Add partition column filter (WHERE partition_col = ...) '
                            'or rewrite query to reference partition column in filter.',
                            table=table,
                            detail=stripped,
                        )
                    )
        ...
        # Skew indicator
        exchanges = [ln.strip() for ln in lines if 'Exchange' in ln and 'partition=' in ln]
        if len(exchanges) > 2:
...
def _detect_exploding_joins(plan_text: str, lines: list[str]) -> list[Finding]:
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'\d+\s+\[.*\]', stripped) and 'Join' in stripped:
            ...
            next_context = '\n'.join(lines[i + 1 : i + 15])
            input_scan_lines = [ln for ln in prev_context.split('\n') if 'Scan' in ln and 'num_rows' in ln.lower()]
            output_join_lines = [ln for ln in next_context.split('\n') if 'Join' in ln]
