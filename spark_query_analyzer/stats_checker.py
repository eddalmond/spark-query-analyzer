...
class StatsFinding:
    ...
    table: str
    suggestion: str
    analyze_command: Optional[str] = None  # copy-ready ANALYZE command
...
class TableStats:
    ...
    has_statistics: bool
    stats_age_hours: Optional[float]  # hours since last ANALYZE
    last_write_timestamp: Optional[str]
    last_analyze_timestamp: Optional[str]
...
def check_table_stats(spark, table: str) -> TableStats:
    ...
    try:
        ...
        for row in col_rows:
            ...
            data_type = str(row.get("data_type", "") or row.get("col_type", "")).strip()
    ...
    for table in plan_tables:
        ...
        if not stats.has_statistics:
            ...
            findings.append(StatsFinding(
                ...
                table=table,
                suggestion=f"Run ANALYZE TABLE to collect statistics:",
        ...
        if stats.is_stale:
            ...
            findings.append(StatsFinding(
                ...
                table=table,
                suggestion=f"Re-run ANALYZE TABLE to refresh statistics:",
                analyze_command=f"ANALYZE TABLE {table} COMPUTE STATISTICS FOR ALL COLUMNS",
        ...
        if stats.row_count and stats.row_count > 100_000_000:
            ...
            if stats.columns_analyzed and stats.stats_age_hours and stats.stats_age_hours > 168:
                findings.append(StatsFinding(
                    ...
                    table=table,
                    suggestion=f"Re-run full column statistics and verify all filter/join columns are included:",
                    analyze_command=f"ANALYZE TABLE {table} COMPUTE STATISTICS FOR ALL COLUMNS",
...
def format_stats_findings_table(findings: list[StatsFinding]) -> str:
    ...
    for f in findings:
        ...
        if f.analyze_command:
            escaped = f.analyze_command.replace("\n", "<br>").replace(" ", "&nbsp;")
            analyze_block = (
                f'<div style="margin-top:6px;padding:8px;background:#1e293b;color:#f8fafc;'
                f'border-radius:6px;font-size:11px;font-family:monospace;">'
                f'<strong style="color:#60a5fa;">&#x2714; Run:</strong><br>{escaped}'
                f'</div>'
            )
