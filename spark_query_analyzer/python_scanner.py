        ...
        if func_name in ('udf', 'pandas_udf', 'register') and node.args:
            self.findings.append(
                PyFinding(
                    ...
                    line=node.lineno,
                    suggestion='Use spark.udf.register() with a SQL expression, or F.col-based expressions instead.',
                    detail=func_name,
                )
            )
        ...
        if func_name in ('collect', 'toPandas'):
            ...
            self.findings.append(
                PyFinding(
                    severity='critical',
                    code='DRIVER_PULL',
                    message=f'.{func_name}() pulls all rows to the driver — OOM risk on large result sets.',
                    line=node.lineno,
        ...
        if attr == 'rdd':
            self.findings.append(
                PyFinding(
                    severity='critical',
                    code='RDD_USAGE',
                    message='.rdd accessed — converts DataFrame to RDD, '
