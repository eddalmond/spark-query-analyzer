...
class ClusterRecommendation:
    ...
    spark_conf: str | None = None  # copy-ready spark.conf.set(...) block
    cluster_policy: str | None = None  # cluster policy JSON snippet
...
def classify_workload(plan_text: str, findings: list = None, python_findings: list = None) -> WorkloadProfile:
    ...
    if python_findings:
        profile.has_python_udf = any(
            'UDF' in getattr(pf, 'code', '') or 'udf' in getattr(pf, 'message', '').lower() for pf in python_findings
        )
...
def recommend_cluster(
    ...
    int(get('spark.executor.instances', '0') or '0') or 2
    ...
    if (
        profile.is_scan_heavy
        and not profile.is_shuffle_heavy
        and not profile.has_python_udf
        and not profile.has_streaming
    ):
        recommendations.append(
            ClusterRecommendation(
                severity='info',
        ...
        recommendations.append(
            ClusterRecommendation(
                ...
                message=(
                    'Shuffle-heavy and Python workloads perform better on Jobs clusters '
                    'with memory-optimised instances (r-family on AWS, Edsv5 on Azure). '
                    'SQL Warehouses are not optimised for large shuffles.'
                ),
            )
        )
    ...
    if current_shuffle_partitions == 200 and profile.estimated_shuffle_mb > 1000:
        ...
        recommendations.append(
            ClusterRecommendation(
                ...
                heading='Default shuffle partitions may cause small tasks',
                message=(
