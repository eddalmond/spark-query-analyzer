...
def _check_trigger_antipatterns(query, trigger_interval: str | None) -> list[StreamingFinding]:
...
def format_streaming_diagnostics(result: StreamingAnalysisResult) -> str:
            ...
            if '```' in f.suggestion:
                ...
                for i, part in enumerate(parts):
                    if i % 2 == 1:  # Code block
                        ...
                        suggestion_html += (
                            f'<div style="font-size:12px;color:#475569;margin-top:4px;">{part.strip()}</div>'
                        )
            else:
    ...
    if result.watermark_configured:
        meta_parts.append('&#x1F4CC; Watermark: configured')
    else:
        meta_parts.append('&#x1F4CC; Watermark: not set')
