...
if TYPE_CHECKING:
    from spark_query_analyzer.analyzer import AnalysisResult
...
def format_diagnostics(
    ...
    delta_results: list = None,
    python_findings: list = None,
    skew_findings: list = None,
    cost_badge: str = '',
    stats_findings: list = None,
    query_signature: str = '',
    history_tracking_html: str = '',
    narrative_result=None,
    cluster_recommendations: list = None,
) -> str:
    """Render an AnalysisResult as a self-contained HTML fragment."""
    counts = result.severity_counts
    total = len(result.findings)
    plural = '' if total == 1 else 's'
    header_title = f'Spark Query Analyzer \u2014 {total} finding{plural}'
    ...
    if not result.findings:
        ...
        for f in result.findings:
            ...
            suggestion_html = (
                f'<div class="sqa-suggestion"><strong>\u2192 Fix:</strong> {f.suggestion}</div>' if f.suggestion else ''
            )
            if f.config_snippet:
    ...
    if python_findings:
        ...
        for pf in python_findings:
            ...
            suggestion_html = (
                f'<div class="sqa-suggestion"><strong>\u2192 Fix:</strong> {pf.suggestion}</div>'
                if pf.suggestion
                else ''
            )
            py_findings_html += (
        ...
        python_html = (
            '<div style="border-top:2px solid #e2e8f0;margin-top:4px;">' + python_header + py_findings_html + '</div>'
        )
    ...
    if delta_results:
        ...
        for dr in delta_results:
            ...
            for df in dr.findings:
                ...
                suggestion_html = (
                    f'<div class="sqa-suggestion"><strong>\u2192 Fix:</strong> {df.suggestion}</div>'
                    if df.suggestion
                    else ''
                )
                delta_findings_html += (
        ...
        if delta_sections:
            delta_html = (
                '<div style="border-top:2px solid #e2e8f0;margin-top:4px;">' + ''.join(delta_sections) + '</div>'
            )
    ...
    if skew_findings:
        ...
        for sf in skew_findings:
            ...
            suggestion_html = (
                f'<div class="sqa-suggestion"><strong>\u2192 Fix:</strong> {sf.suggestion}</div>'
                if sf.suggestion
                else ''
            )
            ui_link_html = (
                ...
                if getattr(sf, 'spark_ui_link', None)
                else ''
            )
            skew_findings_html += (
                f'<div class="sqa-finding" style="border-left-color:{border}">'
        ...
        skew_html = (
            '<div style="border-top:2px solid #e2e8f0;margin-top:4px;">' + skew_header + skew_findings_html + '</div>'
        )
    ...
    if stats_findings:
        ...
        stats_html = (
            '<div style="border-top:2px solid #e2e8f0;margin-top:4px;">' + stats_header + stats_findings_html + '</div>'
        )
...
def display_issue_catalogue() -> None:
    ...
    catalogue = [
        (
            'critical',
            'MISSING_BROADCAST',
            'Broadcast join recommended but not used',
            'Add BROADCAST hint: JOIN ... BROADCAST(t) or increase spark.sql.autoBroadcastJoinThreshold',
        ),
        (
            'critical',
            'CARTESIAN_PRODUCT',
            'Join with no condition \u2014 cross join',
            'Add an explicit JOIN condition or restructure the query',
        ),
        ('high', 'FULL_TABLE_SCAN', 'Scan without partition filter', 'Add partition column to WHERE clause'),
        ('high', 'SORT_MERGE_JOIN', 'Sort-based join on large tables', 'Use BROADCAST or SHUFFLE_HASH hint instead'),
        (
            'high',
            'MISSING_PUSHDOWN',
            'Filter applied after scan instead of during',
            'Rewrite filter to push into scan source',
        ),
        ('medium', 'REPEATED_SCAN', 'Same table scanned multiple times', 'Use CTE or cache the table'),
        (
            'medium',
            'WIDE_TRANSFORM',
            'Wide transformation without LIMIT',
            'Add LIMIT if applicable, or use AQE skew optimisation',
        ),
        (
            'medium',
            'SKEW_INDICATOR',
            'Partition count variance suggests skew',
            'Enable AQE: spark.sql.adaptive.enabled=true',
        ),
        ('info', 'BROADCAST_USED', 'Broadcast join is in use', 'No action needed'),
        ('info', 'PARTITION_PRUNED', 'Partition pruning detected', 'No action needed'),
    ]
