...
class NarrativeResult:
    ...
    def render(self, detailed: bool = False) -> str:
        ...
        if self.is_informational:
            summary = (
                f'<p><strong>Summary:</strong> {self.profile.summary_sentence()}  No performance issues detected.</p>'
            )
        else:
            summary = (
...
class NarrativeExplainer:
    ...
    def _build_profile(self) -> QueryProfile:
        ...
        def _row_count_key(s: str) -> float:
            s = s.upper()
        ...
        tables = re.findall(r'(?:FROM|JOIN)\s+(\w+(?:\.\w+)?)', q, re.IGNORECASE)
    ...
    def _try_llm_explain(self, profile: QueryProfile, detailed: bool) -> NarrativeResult | None:
        """
...
def format_narrative_banner(result: NarrativeResult, detailed: bool = False) -> str:
    ...
    llm_badge = (
        ...
        if result.llm_used
        else '<div class="sqa-narrative-badge">&#x1F4DD; Summary</div>'
    )
    return f"<style>{_SUMMARY_CSS}</style><div class='sqa-narrative'>{llm_badge}{narrative_html}</div>"
